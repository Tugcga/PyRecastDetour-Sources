...
public:
	...
	void set_mode_streight(int vertex_mode) 
	{
		m_toolMode = TOOLMODE_PATHFIND_STRAIGHT;
		m_straightPathOptions = vertex_mode;
	}

	void set_mode_distance(){m_toolMode = TOOLMODE_DISTANCE_TO_WALL;}
	void set_mode_raycast() { m_toolMode = TOOLMODE_RAYCAST; }

	void set_points(const float* s, const float* p);
	void set_point(const float* p);

	int get_namesh_polys_count() { return m_npolys; }
	int get_path_points_count(){ return m_nstraightPath;}
	float get_distance_to_wall(){return m_distanceToWall;}
	float* get_path(){return &m_straightPath[0];}
};
